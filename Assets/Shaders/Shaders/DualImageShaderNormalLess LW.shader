Shader "TriplanarUltimate/DualSurfaceShaderNormalLessLightWeight" {
    ...
    Properties{
        ...
        _SmoothEnd("Smooth+Metal Width", Range(0, 1)) = 0.5                                             // dictated by the height of the interpolator texture.
        ...
        SubShader{
            ...
            void surf(Input IN, inout SurfaceOutputStandard o) {
                ...
                float2x2 mat2 = triblend.x == 0 ? float2x2(_SecCos.y, -_SecSin.y, _SecSin.y, _SecCos.y) : float2x2(_SecCos.x, -_SecSin.x, _SecSin.x, _SecCos.x); mat2 = triblend.z == 0 ? mat2 : float2x2(_SecCos.z, -_SecSin.z, _SecSin.z, _SecCos.z);
                ...
                float2 uvZ2 = IN.worldPos.xy * _SecScaleFactor.xy + _SecOffsetFactor.xy;                            //So if you were wondering what those W values did, it was absolutely nothing, unity only lets you display float4s for some reason 
                ...
                float2 uvX4 = IN.worldPos.zy * _SecIntScaleFactor.zy + _SecIntOffsetFactor.zy;                      //And again to add another layer, break up repeating interp textures;
                float2 uvY4 = IN.worldPos.xz * _SecIntScaleFactor.xz + _SecIntOffsetFactor.xz;
                float2 uvZ4 = IN.worldPos.xy * _SecIntScaleFactor.xy + _SecIntOffsetFactor.xy;
                ...
                float2 uv2 = triblend.x == 0 ? uvY2 : uvX2; uv2 = triblend.z == 0 ? uv2 : uvZ2;                     //This reduces the max possible texture samples from 24 to 8
                float2 uv3 = triblend.x == 0 ? uvY3 : uvX3; uv3 = triblend.z == 0 ? uv3 : uvZ3;                     
                uv3 = mul(mat1, uv3);
                float2 uv4 = triblend.x == 0 ? uvY4 : uvX4; uv4 = triblend.z == 0 ? uv4 : uvZ4;
                uv4 = mul(mat2, uv4);

                //Interpolator Bump Texture value
                float dn = lerp(tex2D(_InterpBump, uv3), tex2D(_SecInterpBump, uv4), _IntBlend);                    //Unclamped texture for more consistent reference points, Some n values would cause things to break lol
                ...
                fixed4 col = tex2D(_MainTex, uv1);              //Sampling albedo maps
                fixed4 c2ol = tex2D(_UnderTex, uv2);            //Only 4 texture samples in this shader, we are really getting barebones here lol
