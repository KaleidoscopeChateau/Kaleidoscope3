Shader "TriplanarUltimate/SingleSurfaceShader" {
    ...
    Properties{
        ...
        SubShader{
            ...
            void surf(Input IN, inout SurfaceOutputStandard o) {
                ...
                half3 tnormalX = B1X;        //Interpolating between the different normal maps based on the interp value
                half3 tnormalY = B1Y;
                half3 tnormalZ = B1Z;
                ...
                if (n - int(n) != 0)
                {
                    inormalX = UnpackNormal(tex2D(_InterpNormal, uvX3));                                                                    //Interpolating between interpolator normal texture values
                    inormalY = UnpackNormal(tex2D(_InterpNormal, uvY3));                                                                    //Causes occasional incorrect looking normal maps, but the alternative was to calculate the combined normal of 
                    inormalZ = UnpackNormal(tex2D(_InterpNormal, uvZ3));                                                                    //The 2 bump maps on the fly, which would have been a lot more buggy, unstable, and complex
                ...
                float4 ccc = col * cc;                                                                                                                          //Lerping between the 2 Base colours. sorry about my naming conventions lol  
