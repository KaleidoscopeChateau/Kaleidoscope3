...
public class DualSurfaceShaderEditor : MaterialEditor
{
    bool uniform;
    bool uniform2;
    bool uniform3;
        ...
        private void OnSceneGUI()
    {
        ...
        if (EditorWindow.mouseOverWindow != null && boxedit && onceagain)
        {
        ...
        if (t.GetFloat("_IntPosInf") == 1)
        {
            ...
            if (place)
            {
                ...
                Handles.DrawLine(tryplace, pos, 0.1f);
                if (Camera.current != null) 
                Handles.DrawSolidDisc(tryplace, Camera.current.transform.forward, 0.1f);
            ...
            if (boxedit | gradedit)
            {
                ...
                mousePosition.y = cam.camera.pixelHeight - mousePosition.y * ppp;
                mousePosition.x *= ppp;
                float dist = 10000;
                ...
                if (active != -1)
                {
                    if (active % 2 == 0)
                    {
                        pos = (points[active] + points[active + 1]) / 2;
                    }
                    else
                    {
                        pos = (points[active] + points[active - 1]) / 2;
                ...
                Handles.color = Color.green;
                if(Camera.current != null && boxedit)
                {
                    if (active != -1)
                ...
                if (e.button == 0 && e.isMouse && !once && e.type == EventType.MouseDown)
                {
                    ...
                    for (int i = 0; i < points.Count -1; i++)
                    {
                        ...
                        float curDist = Vector2.Distance(new Vector2(screenPos.x, screenPos.y), new Vector2(mousePosition.x, mousePosition.y));
                ...
                if (e.button == 0 && e.isMouse && once && e.type == EventType.MouseUp)
                    once = false;
            }
        }
    }
